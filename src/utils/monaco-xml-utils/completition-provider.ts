...
export function stringToXml(text: string): Document {
...
function getItemDocumentation(element: Element): string | undefined {
  const children = [...element.children]
  for (const child of children) {
    if (child.tagName === 'xs:annotation') {
      return getItemDocumentation(element.children[0] as Element)
...
const getAvailableElements = (element: Element) => {
  const complexType = [...element.children].find(c => c.tagName === 'xs:complexType')
  ...
  const sequences = [...complexType.children].filter(child => child.tagName === 'xs:sequence')
  const allowedElements = [
    ...sequences
      .map(s => [...s.children].filter(child => child.tagName === 'xs:element'))
      .reduce((acc, val) => acc.concat(val)),
  ]

  return {
    suggestions: allowedElements.map(el => {
...
const getAvailableAttribute = (element: Element, usedAttributes: string[]) => {
  const complexTypeElement = [...element.children].find(a => a.tagName === 'xs:complexType')
...
export function getXsdCompletionProvider(schemaString: string): monaco.languages.CompletionItemProvider {
  return {
    ...
    provideCompletionItems: async (model, position) => {
      ...
      const usedAttributes: string[] = outerXmlNode
        .split(' ')
        .map(n => RegExp(/(\S+)=(["'])(\S)([>"'])?/).exec(n))
        .filter(a => a != null)
        .map(a => (a && a[1]) || '')
      ...
      if (isAttributeSearch) {
        ...
        return currentItem ? getAvailableAttribute(currentItem, usedAttributes) : { suggestions: [] }
      } else {
        // get elements completions
        return currentItem ? getAvailableElements(currentItem) : { suggestions: [] }
      }
    },
  }
}
